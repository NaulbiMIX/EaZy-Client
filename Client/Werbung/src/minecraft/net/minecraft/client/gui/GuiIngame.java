...
extends Gui {
    ...
    public void func_175180_a(float p_175180_1_) {
        ...
        if (!this.mc.thePlayer.isPotionActive(Potion.confusion) && (var6 = this.mc.thePlayer.prevTimeInPortal + (this.mc.thePlayer.timeInPortal - this.mc.thePlayer.prevTimeInPortal) * p_175180_1_) > 0.0f) {
        ...
        if (this.mc.gameSettings.keyBindPlayerList.getIsKeyPressed() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || var16 != null)) {
    ...
    private void renderArrayList() {
        ...
        for (Module m : Werbung.instance.modulemanager.getModules()) {
            if (!m.isEnabled() || !m.visible || m.isCategory(Category.GUI)) continue;
            display.add(String.valueOf(m.getDisplayname()));
        }
        Collections.sort(Werbung.instance.modulemanager.getModules(), new Comparator(){

            public int compare(Object mo1, Object mo2) {
                Module mod1 = (Module)mo1;
                Module mod2 = (Module)mo2;
                if (fr.getStringWidth(mod1.getDisplayname()) > fr.getStringWidth(mod2.getDisplayname())) {
                    return -1;
                }
                return fr.getStringWidth(mod1.getDisplayname()) < fr.getStringWidth(mod2.getDisplayname());
            }
        });
        for (String s : display) {
            ...
            int mhight = 10 * i + 2;
            fr.drawString(s, mwidth, mhight, 16777215);
            ++i;
            ++x;
        }
        display.clear();
    ...
    private void renderName() {
        ...
        fr.drawString(String.valueOf(Werbung.instance.Client_Name) + " | " + Werbung.instance.Client_Version, 3, 5, 16777215);
    }
