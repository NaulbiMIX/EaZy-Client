...
public class Checksum {
    ...
    public static long queryChecksum(byte[] buf, int len) {
        ...
        int j = 0;
        for (int k = 0; k < len; ++k) {
            i += (j += single_hash[buf[k] + 128]);
        }
        return (i & 65535) << 16 | j & 65535;
    }
    ...
    public void generateChecksums(InputStream sis, int length) throws IOException {
        BufferedInputStream inputstream = new BufferedInputStream(sis);
        int i = length / 16;
        if (debug) {
        ...
        this.prime = Checksum.findClosestPrime(i);
        if (debug) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            this.checksums[j] = Checksum.queryChecksum(abyte, 16);
        }
        for (int k = 0; k < i; ++k) {
            this.hashtable[k] = -1;
        }
        for (int l = 0; l < i; ++l) {
            int i1 = Checksum.generateHash(this.checksums[l]) % this.prime;
            if (debug) {
