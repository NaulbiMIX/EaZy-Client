    ...
    private void checkTimeout() {
        if (this.socket != null) {
            ...
            if ((j = System.currentTimeMillis()) > this.timeLastActivityMs + i) {
                this.terminate((Exception)((Object)new InterruptedException("Timeout " + i)));
            }
        }
    }
            ...
            if (this.listRequests.size() > 0 && this.listRequests.get(0) == pr) {
                ...
                if (resp.getStatus() / 100 == 3 && s != null && pr.getHttpRequest().getRedirects() < 5) {
                    try {
                        ...
                        pr.getHttpListener().failed(pr.getHttpRequest(), (Exception)((Object)ioexception));
                    }
                } else {
    ...
    private void checkResponseHeader(HttpResponse resp) {
        ...
        if ((s1 = resp.getHeader("Keep-Alive")) != null) {
            ...
            for (int i = 0; i < astring.length; ++i) {
                ...
                if (astring1[0].equals((Object)"timeout") && (j = Config.parseInt(astring1[1], -1)) > 0) {
                    this.keepaliveTimeoutMs = j * 1000;
                }
                if (!astring1[0].equals((Object)"max") || (k = Config.parseInt(astring1[1], -1)) <= 0) continue;
