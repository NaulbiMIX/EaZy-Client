...
public class CustomColors {
    ...
    private static final IColorizer COLORIZER_WATER = new IColorizer(){
        ...
        public int getColor(IBlockState blockState, IBlockAccess blockAccess, BlockPos blockPos) {
            BiomeGenBase biomegenbase = CustomColors.getColorBiome(blockAccess, blockPos);
            if (waterColors != null) {
                return waterColors.getColor(biomegenbase, blockPos);
            }
            return Reflector.ForgeBiome_getWaterColorMultiplier.exists() ? Reflector.callInt(biomegenbase, Reflector.ForgeBiome_getWaterColorMultiplier, new Object[0]) : biomegenbase.waterColorMultiplier;
        }
    ...
    private static CustomColormap[][] blockListToArray(List list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (l == null) continue;
            CustomColormap[] acustomcolormap1 = (CustomColormap[])l.toArray((Object[])new CustomColormap[l.size()]);
    ...
    private static int parseColor(String str) {
        ...
        str = str.trim();
        try {
            int i = Integer.parseInt((String)str, (int)16) & 16777215;
            return i;
        }
    ...
    public static int getXpOrbColor(float timer) {
        ...
        int i = (int)Math.round((double)((double)((MathHelper.sin(timer) + 1.0f) * (float)(xpOrbColors.getLength() - 1)) / 2.0));
    ...
    private static int getSpawnEggColor(ItemMonsterPlacer item, ItemStack itemStack, int layer, int color) {
        ...
        int[] arrn = aint = layer == 0 ? spawnEggPrimaryColors : spawnEggSecondaryColors;
        if (aint == null) {
            return color;
        }
    ...
    public static int getColorFromItemStack(ItemStack itemStack, int layer, int color) {
        ...
        return item instanceof ItemMonsterPlacer ? CustomColors.getSpawnEggColor((ItemMonsterPlacer)item, itemStack, layer, color) : color;
    }
