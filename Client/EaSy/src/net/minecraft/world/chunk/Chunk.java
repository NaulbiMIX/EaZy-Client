...
public class Chunk {
    ...
    private final ClassInheritanceMultiMap<Entity>[] entityLists = new ClassInheritanceMultiMap[16];
            ...
            for (int i = 0; i < 16; ++i) {
                for (int j = 0; j < 16; ++j) {
                    ...
                    for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
                        j1 = Math.min((int)j1, (int)this.worldObj.getChunksLowestHorizon(l + enumfacing.getFrontOffsetX(), i1 + enumfacing.getFrontOffsetZ()));
                    }
    ...
    private void relightBlock(int x, int y, int z) {
        int i;
        int j = i = this.heightMap[z << 4 | x] & 255;
        if (y > i) {
            j = y;
        }
    ...
    public IBlockState setBlockState(BlockPos pos, IBlockState state) {
        ...
        int i = pos.getX() & 15;
        int j = pos.getY();
        if (j >= this.precipitationHeightMap[l = (k = pos.getZ() & 15) << 4 | i] - 1) {
    ...
    public boolean canSeeSky(BlockPos pos) {
        ...
        int i = pos.getX() & 15;
        int j = pos.getY();
        return j >= this.heightMap[(k = pos.getZ() & 15) << 4 | i];
    }
    ...
    public void onChunkLoad() {
        ...
        this.worldObj.addTileEntities((Collection<TileEntity>)this.chunkTileEntityMap.values());
        for (int i = 0; i < this.entityLists.length; ++i) {
            Iterator<Entity> iterator = this.entityLists[i].iterator();
            while (iterator.hasNext()) {
                Entity entity = (Entity)iterator.next();
                entity.onChunkLoad();
            }
            this.worldObj.loadEntities((Collection<Entity>)this.entityLists[i]);
        }
    }
    ...
    public void onChunkUnload() {
        ...
        for (int i = 0; i < this.entityLists.length; ++i) {
            this.worldObj.unloadEntities((Collection<Entity>)this.entityLists[i]);
        }
    }
    ...
    public void getEntitiesWithinAABBForEntity(Entity entityIn, AxisAlignedBB aabb, List<Entity> listToFill, Predicate<? super Entity> p_177414_4_) {
        ...
        for (int k = i; k <= j; ++k) {
            ...
            Iterator<Entity> iterator = this.entityLists[k].iterator();
            while (iterator.hasNext()) {
                Entity[] aentity;
                Entity entity = (Entity)iterator.next();
