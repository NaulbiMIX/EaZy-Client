...
implements ITileEntityProvider {
    ...
    private int calculateOutput(World worldIn, BlockPos pos, IBlockState state) {
        return state.getValue(MODE) == Mode.SUBTRACT ? Math.max((int)(this.calculateInputStrength(worldIn, pos, state) - this.getPowerOnSides(worldIn, pos, state)), (int)0) : this.calculateInputStrength(worldIn, pos, state);
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        worldIn.playSoundEffect((double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5, "random.click", 0.3f, (state = state.cycleProperty(MODE)).getValue(MODE) == Mode.SUBTRACT ? 0.55f : 0.5f);
