...
implements IProjectile {
    ...
    public EntityArrow(World worldIn, EntityLivingBase shooter, EntityLivingBase target, float velocity, float innacuracy) {
        ...
        this.posY = shooter.posY + (double)shooter.getEyeHeight() - 0.10000000149011612;
        double d0 = target.posX - shooter.posX;
        double d1 = target.getEntityBoundingBox().minY + (double)(target.height / 3.0f) - this.posY;
        double d2 = target.posZ - shooter.posZ;
        double d3 = MathHelper.sqrt_double(d0 * d0 + d2 * d2);
        if (!(d3 < 1.0E-7)) {
            float f = (float)(MathHelper.atan2(d2, d0) * 180.0 / 3.1415927410125732) - 90.0f;
            float f1 = (float)(-(MathHelper.atan2(d1, d3) * 180.0 / 3.1415927410125732));
            double d4 = d0 / d3;
            double d5 = d2 / d3;
            this.setLocationAndAngles(shooter.posX + d4, this.posY, shooter.posZ + d5, f, f1);
            float f2 = (float)(d3 * 0.20000000298023224);
    ...
    public void setThrowableHeading(double x, double y, double z, float velocity, float inaccuracy) {
        ...
        y /= (double)f;
        z /= (double)f;
        x += this.rand.nextGaussian() * (double)(this.rand.nextBoolean() ? -1 : 1) * 0.007499999832361937 * (double)inaccuracy;
        y += this.rand.nextGaussian() * (double)(this.rand.nextBoolean() ? -1 : 1) * 0.007499999832361937 * (double)inaccuracy;
        z += this.rand.nextGaussian() * (double)(this.rand.nextBoolean() ? -1 : 1) * 0.007499999832361937 * (double)inaccuracy;
        this.motionX = x *= (double)velocity;
        this.motionY = y *= (double)velocity;
        this.motionZ = z *= (double)velocity;
        float f1 = MathHelper.sqrt_double(x * x + z * z);
        this.prevRotationYaw = this.rotationYaw = (float)(MathHelper.atan2(x, z) * 180.0 / 3.1415927410125732);
        this.prevRotationPitch = this.rotationPitch = (float)(MathHelper.atan2(y, f1) * 180.0 / 3.1415927410125732);
    ...
    public void setVelocity(double x, double y, double z) {
        ...
        if (this.prevRotationPitch == 0.0f && this.prevRotationYaw == 0.0f) {
            ...
            this.prevRotationYaw = this.rotationYaw = (float)(MathHelper.atan2(x, z) * 180.0 / 3.1415927410125732);
            this.prevRotationPitch = this.rotationPitch = (float)(MathHelper.atan2(y, f) * 180.0 / 3.1415927410125732);
    ...
    public void onUpdate() {
        ...
        if (this.prevRotationPitch == 0.0f && this.prevRotationYaw == 0.0f) {
            ...
            this.prevRotationPitch = this.rotationPitch = (float)(MathHelper.atan2(this.motionY, f) * 180.0 / 3.1415927410125732);
        }
        ...
        if (this.inGround) {
            ...
            if (movingobjectposition != null) {
                if (movingobjectposition.entityHit != null) {
                    ...
                    if (movingobjectposition.entityHit.attackEntityFrom(damagesource, l)) {
                        if (movingobjectposition.entityHit instanceof EntityLivingBase) {
                            ...
                            if (this.knockbackStrength > 0 && (f7 = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ)) > 0.0f) {
                                movingobjectposition.entityHit.addVelocity(this.motionX * (double)this.knockbackStrength * 0.6000000238418579 / (double)f7, 0.1, this.motionZ * (double)this.knockbackStrength * 0.6000000238418579 / (double)f7);
                            }
                    ...
                    this.posX -= this.motionX / (double)f5 * 0.05000000074505806;
                    this.posY -= this.motionY / (double)f5 * 0.05000000074505806;
                    this.posZ -= this.motionZ / (double)f5 * 0.05000000074505806;
                    this.playSound("random.bowhit", 1.0f, 1.2f / (this.rand.nextFloat() * 0.2f + 0.9f));
                    this.inGround = true;
                    this.arrowShake = 7;
            ...
            this.rotationPitch = (float)(MathHelper.atan2(this.motionY, f3) * 180.0 / 3.1415927410125732);
