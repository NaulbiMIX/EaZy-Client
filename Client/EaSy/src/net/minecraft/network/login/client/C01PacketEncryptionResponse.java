...
implements Packet<INetHandlerLoginServer> {
    ...
    public C01PacketEncryptionResponse(SecretKey secretKey, PublicKey publicKey, byte[] verifyToken) {
        this.secretKeyEncrypted = CryptManager.encryptData((Key)publicKey, secretKey.getEncoded());
        this.verifyTokenEncrypted = CryptManager.encryptData((Key)publicKey, verifyToken);
    }
    ...
    public byte[] getVerifyToken(PrivateKey key) {
        return key == null ? this.verifyTokenEncrypted : CryptManager.decryptData((Key)key, this.verifyTokenEncrypted);
    }
}
