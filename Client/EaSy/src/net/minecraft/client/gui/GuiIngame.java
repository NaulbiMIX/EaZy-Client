...
extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        EaSyClient.INSTANCE.getHudRenderer().renderHud(1, 1);
        if (!EaSyClient.INSTANCE.demoMode) {
            this.mc.displayGuiScreen(new GuiScreenDemo());
            EaSyClient.INSTANCE.demoMode = true;
        }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            GuiIngame.drawRect(l1 - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            this.getFontRenderer().drawString(s1, l1, k, 553648127);
            this.getFontRenderer().drawString(s2, l - this.getFontRenderer().getStringWidth(s2), k, 553648127);
            if (j != collection.size()) continue;
            String s3 = objective.getDisplayName();
            GuiIngame.drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 1610612736);
            GuiIngame.drawRect(l1 - 2, k - 1, l, k, 1342177280);
