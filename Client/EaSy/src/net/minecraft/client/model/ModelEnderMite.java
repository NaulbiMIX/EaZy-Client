...
extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        for (int i = 0; i < this.field_178713_d.length; ++i) {
            this.field_178713_d[i].rotateAngleY = MathHelper.cos(ageInTicks * 0.9f + (float)i * 0.15f * 3.1415927f) * 3.1415927f * 0.01f * (float)(1 + Math.abs((int)(i - 2)));
            this.field_178713_d[i].rotationPointX = MathHelper.sin(ageInTicks * 0.9f + (float)i * 0.15f * 3.1415927f) * 3.1415927f * 0.1f * (float)Math.abs((int)(i - 2));
        }
    }
}
