...
extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) {
            return CommandEffect.getListOfStringsMatchingLastWord(args, this.getAllUsernames());
        }
        if (args.length == 2) {
            return CommandEffect.getListOfStringsMatchingLastWord(args, Potion.getPotionLocations());
        }
        return args.length == 5 ? CommandEffect.getListOfStringsMatchingLastWord(args, "true", "false") : null;
    }
