...
extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) {
            return CommandExecuteAt.getListOfStringsMatchingLastWord(args, MinecraftServer.getServer().getAllUsernames());
        }
        if (args.length > 1 && args.length <= 4) {
            return CommandExecuteAt.func_175771_a(args, 1, pos);
        }
        if (args.length > 5 && args.length <= 8 && "detect".equals((Object)args[4])) {
            return CommandExecuteAt.func_175771_a(args, 5, pos);
        }
        return args.length == 9 && "detect".equals((Object)args[4]) ? CommandExecuteAt.getListOfStringsMatchingLastWord(args, Block.blockRegistry.getKeys()) : null;
    }
