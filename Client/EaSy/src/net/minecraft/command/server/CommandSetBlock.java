...
extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length > 0 && args.length <= 3) {
            return CommandSetBlock.func_175771_a(args, 0, pos);
        }
        if (args.length == 4) {
            return CommandSetBlock.getListOfStringsMatchingLastWord(args, Block.blockRegistry.getKeys());
        }
        return args.length == 6 ? CommandSetBlock.getListOfStringsMatchingLastWord(args, "replace", "destroy", "keep") : null;
    }
}
