...
extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length > 0 && args.length <= 3) {
            return CommandCompare.func_175771_a(args, 0, pos);
        }
        if (args.length > 3 && args.length <= 6) {
            return CommandCompare.func_175771_a(args, 3, pos);
        }
        if (args.length > 6 && args.length <= 9) {
            return CommandCompare.func_175771_a(args, 6, pos);
        }
        return args.length == 10 ? CommandCompare.getListOfStringsMatchingLastWord(args, "masked", "all") : null;
    }
}
