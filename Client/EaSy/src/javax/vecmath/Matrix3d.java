...
Cloneable {
    ...
    static boolean luDecomposition(double[] matrix0, int[] row_perm) {
        ...
        while (i-- != 0) {
            ...
            while (j-- != 0) {
                ...
                if (!((temp = Math.abs((double)temp)) > big)) continue;
                big = temp;
            }
            if (big == 0.0) {
                return false;
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            for (i2 = j; i2 < 3; ++i2) {
                ...
                matrix0[target] = sum;
                temp = row_scale[i2] * Math.abs((double)sum);
                if (!(d >= big)) continue;
                big = temp;
                imax = i2;
            }
    ...
    public final void rotY(double angle) {
        double cosAngle;
        double sinAngle = Math.sin((double)angle);
        this.m00 = cosAngle = Math.cos((double)angle);
        this.m01 = 0.0;
        this.m02 = sinAngle;
        this.m10 = 0.0;
        this.m11 = 1.0;
    ...
    public final void normalizeCP() {
        double mag = 1.0 / Math.sqrt((double)(this.m00 * this.m00 + this.m10 * this.m10 + this.m20 * this.m20));
        this.m00 *= mag;
        this.m10 *= mag;
        this.m20 *= mag;
        mag = 1.0 / Math.sqrt((double)(this.m01 * this.m01 + this.m11 * this.m11 + this.m21 * this.m21));
        this.m01 *= mag;
        this.m11 *= mag;
        this.m21 *= mag;
    ...
    public final void normalizeCP(Matrix3d m1) {
        double mag = 1.0 / Math.sqrt((double)(m1.m00 * m1.m00 + m1.m10 * m1.m10 + m1.m20 * m1.m20));
        this.m00 = m1.m00 * mag;
        this.m10 = m1.m10 * mag;
        this.m20 = m1.m20 * mag;
        mag = 1.0 / Math.sqrt((double)(m1.m01 * m1.m01 + m1.m11 * m1.m11 + m1.m21 * m1.m21));
        this.m01 = m1.m01 * mag;
        this.m11 = m1.m11 * mag;
        this.m21 = m1.m21 * mag;
        this.m02 = this.m10 * this.m21 - this.m11 * this.m20;
    ...
    static void compute_svd(double[] m, double[] outScale, double[] outRot) {
        ...
        if (m[3] * m[3] < 1.110223024E-16) {
            ...
            g = 1.0 / Math.sqrt((double)(m[0] * m[0] + m[3] * m[3]));
            double c1 = m[0] * g;
            double s1 = m[3] * g;
            tmp[0] = c1 * m[0] + s1 * m[3];
            tmp[1] = c1 * m[1] + s1 * m[4];
        ...
        if (!(m[6] * m[6] < 1.110223024E-16)) {
            if (m[0] * m[0] < 1.110223024E-16) {
                ...
                g = 1.0 / Math.sqrt((double)(m[0] * m[0] + m[6] * m[6]));
                double c2 = m[0] * g;
                double s2 = m[6] * g;
                tmp[0] = c2 * m[0] + s2 * m[6];
                tmp[1] = c2 * m[1] + s2 * m[7];
        ...
        if (m[2] * m[2] < 1.110223024E-16) {
            ...
            g = 1.0 / Math.sqrt((double)(m[1] * m[1] + m[2] * m[2]));
            double c3 = m[1] * g;
            double s3 = m[2] * g;
            tmp[1] = c3 * m[1] + s3 * m[2];
            m[2] = -s3 * m[1] + c3 * m[2];
        ...
        if (!(m[7] * m[7] < 1.110223024E-16)) {
            if (m[4] * m[4] < 1.110223024E-16) {
                ...
                g = 1.0 / Math.sqrt((double)(m[4] * m[4] + m[7] * m[7]));
                double c4 = m[4] * g;
                double s4 = m[7] * g;
                tmp[3] = c4 * m[3] + s4 * m[6];
                m[6] = -s4 * m[3] + c4 * m[6];
        ...
        if (Matrix3d.almostEqual(Math.abs((double)scales[0]), 1.0) && Matrix3d.almostEqual(Math.abs((double)scales[1]), 1.0) && Matrix3d.almostEqual(Math.abs((double)scales[2]), 1.0)) {
            for (i = 0; i < 3; ++i) {
    ...
    static void svdReorder(double[] m, double[] t1, double[] t2, double[] scales, double[] outRot, double[] outScale) {
        ...
        if (Matrix3d.almostEqual(Math.abs((double)scales[0]), Math.abs((double)scales[1])) && Matrix3d.almostEqual(Math.abs((double)scales[1]), Math.abs((double)scales[2]))) {
            int i;
            for (i = 0; i < 9; ++i) {
                outRot[i] = rot[i];
            }
    ...
    static int compute_qr(double[] s, double[] e, double[] u, double[] v) {
        ...
        int first = 1;
        if (Math.abs((double)e[1]) < 4.89E-15 || Math.abs((double)e[0]) < 4.89E-15) {
            converged = true;
        }
        for (int k = 0; k < 10 && !converged; ++k) {
            double shift = Matrix3d.compute_shift(s[1], e[1], s[2]);
            double f = (Math.abs((double)s[0]) - shift) * (Matrix3d.d_sign(c_b48, s[0]) + shift / s[0]);
            double g = e[0];
            ...
            m[7] = 0.0;
            m[8] = s[2];
            if (!(Math.abs((double)e[1]) < 4.89E-15) && !(Math.abs((double)e[0]) < 4.89E-15)) continue;
            converged = true;
        }
        if (Math.abs((double)e[1]) < 4.89E-15) {
    ...
    static double compute_shift(double f, double g, double h) {
        double ssmin;
        double fa = Math.abs((double)f);
        double ga = Math.abs((double)g);
        double ha = Math.abs((double)h);
        ...
        if (fhmn == 0.0) {
            ...
            double d__1 = ga / fhmx;
            double au = d__1 * d__1;
            double c = 2.0 / (Math.sqrt((double)(as * as + au)) + Math.sqrt((double)(at * at + au)));
            ssmin = fhmn * c;
        } else {
            double au = fhmx / ga;
            if (au == 0.0) {
                ...
                double d__1 = as * au;
                double d__2 = at * au;
                double c = 1.0 / (Math.sqrt((double)(d__1 * d__1 + 1.0)) + Math.sqrt((double)(d__2 * d__2 + 1.0)));
                ssmin = fhmn * c * au;
                ssmin += ssmin;
            }
        }
    ...
    static int compute_2X2(double f, double g, double h, double[] single_values, double[] snl, double[] csl, double[] snr, double[] csr, int index) {
        ...
        if ((ga = Math.abs((double)(gt = g))) == 0.0) {
            single_values[1] = ha;
            single_values[0] = fa;
            clt = 1.0;
            crt = 1.0;
            ...
            if (gasmal) {
                ...
                double mm = m * m;
                double tt = t * t;
                double s = Math.sqrt((double)(tt + mm));
                double r = l == 0.0 ? Math.abs((double)m) : Math.sqrt((double)(l * l + mm));
                double a = (s + r) * 0.5;
                if (ga > fa) {
                    pmax = 2;
                    if (fa / ga < 1.110223024E-16) {
    ...
    static double compute_rot(double f, double g, double[] sin, double[] cos, int index, int first) {
        ...
        if (g == 0.0) {
            ...
            double f1 = f;
            double g1 = g;
            double scale = Matrix3d.max(Math.abs((double)f1), Math.abs((double)g1));
            if (scale >= 4.9947976805055876E145) {
                int count = 0;
                while (scale >= 4.9947976805055876E145) {
                    ++count;
                    scale = Matrix3d.max(Math.abs((double)(f1 *= 2.002083095183101E-146)), Math.abs((double)(g1 *= 2.002083095183101E-146)));
                }
                r = Math.sqrt((double)(f1 * f1 + g1 * g1));
                cs = f1 / r;
                sn = g1 / r;
                int i__1 = count;
                ...
                while (scale <= 2.002083095183101E-146) {
                    ++count;
                    scale = Matrix3d.max(Math.abs((double)(f1 *= 4.9947976805055876E145)), Math.abs((double)(g1 *= 4.9947976805055876E145)));
                }
                r = Math.sqrt((double)(f1 * f1 + g1 * g1));
                cs = f1 / r;
                sn = g1 / r;
            ...
            if (Math.abs((double)f) > Math.abs((double)g) && cs < 0.0) {
                cs = -cs;
                sn = -sn;
                r = -r;
            }
    ...
    private static final boolean almostEqual(double a, double b) {
        ...
        double diff = Math.abs((double)(a - b));
        double absA = Math.abs((double)a);
        double d = max = absA >= (absB = Math.abs((double)b)) ? absA : absB;
        if (diff < 1.0E-6) {
            return true;
        }
