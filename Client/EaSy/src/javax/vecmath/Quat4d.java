...
implements Serializable {
    ...
    public Quat4d(double x, double y, double z, double w) {
        double mag = 1.0 / Math.sqrt((double)(x * x + y * y + z * z + w * w));
        this.x = x * mag;
        this.y = y * mag;
        this.z = z * mag;
        this.w = w * mag;
    ...
    public Quat4d(double[] q) {
        double mag = 1.0 / Math.sqrt((double)(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]));
        this.x = q[0] * mag;
        this.y = q[1] * mag;
        this.z = q[2] * mag;
        this.w = q[3] * mag;
    ...
    public final void normalize(Quat4d q1) {
        ...
        if (norm > 0.0) {
            norm = 1.0 / Math.sqrt((double)norm);
            this.x = norm * q1.x;
            this.y = norm * q1.y;
            this.z = norm * q1.z;
            this.w = norm * q1.w;
    ...
    public final void set(AxisAngle4f a) {
        ...
        if (amag < 1.0E-12) {
            ...
            double mag = Math.sin((double)((double)a.angle / 2.0));
            amag = 1.0 / amag;
            this.w = Math.cos((double)((double)a.angle / 2.0));
    ...
    public final void interpolate(Quat4d q1, double alpha) {
        ...
        if (1.0 - dot > 1.0E-12) {
            double om = Math.acos((double)dot);
            double sinom = Math.sin((double)om);
            s1 = Math.sin((double)((1.0 - alpha) * om)) / sinom;
