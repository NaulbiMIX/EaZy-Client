...
Cloneable {
    ...
    public final void normalize(GVector v1) {
        ...
        double invMag = 1.0 / Math.sqrt((double)sq);
        for (i = 0; i < this.length; ++i) {
    ...
    public final void mul(GMatrix m1, GVector v1) {
        ...
        for (int j = this.length - 1; j >= 0; --j) {
            ...
            for (int i = v1.length - 1; i >= 0; --i) {
                double[] arrd = this.values;
                int n = j;
                arrd[n] = arrd[n] + m1.values[j][i] * v[i];
            }
        }
    }
    ...
    public final void mul(GVector v1, GMatrix m1) {
        ...
        for (int j = this.length - 1; j >= 0; --j) {
            ...
            for (int i = v1.length - 1; i >= 0; --i) {
                double[] arrd = this.values;
                int n = j;
                arrd[n] = arrd[n] + m1.values[i][j] * v[i];
            }
        }
    }
    ...
    public final double angle(GVector v1) {
        return Math.acos((double)(this.dot(v1) / (this.norm() * v1.norm())));
    }
