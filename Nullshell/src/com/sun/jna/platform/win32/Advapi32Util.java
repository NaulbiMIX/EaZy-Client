    ...
    public static Account[] getCurrentUserGroups() {
        ...
        try {
            ...
            if (!Advapi32.INSTANCE.OpenThreadToken(threadHandle, 10, true, phToken)) {
                ...
                if (!Advapi32.INSTANCE.OpenProcessToken((WinNT.HANDLE)processHandle, 10, phToken)) {
                    throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
                }
            }
            processHandle = Advapi32Util.getTokenGroups(phToken.getValue());
    ...
    public static String[] registryGetStringArray(WinReg.HKEY root, String key, String value) {
        ...
        try {
            ...
            while ((long)offset < data.size()) {
                s = data.getString((long)offset, true);
                offset += s.length() * Native.WCHAR_SIZE;
                offset += Native.WCHAR_SIZE;
                result.add(s);
            }
            s = result.toArray(new String[0]);
            return s;
        }
        finally {
            rc = Advapi32.INSTANCE.RegCloseKey(phkKey.getValue());
